...
class ConfigurableAgent:
    ...
    def _get_tools_for_role(self, tool_names: list[str]) -> list[Any]:
        ...
        if hasattr(self.context.tool_registry, '_discovered') and not self.context.tool_registry._discovered:
            logger.info("[AGENT FACTORY] Tool registry not discovered, running auto-discovery...")
        ...
        for tool_name in tool_names:
            ...
            if tool_metadata and tool_metadata.tool_function:
                # Use tools directly - no wrapping needed since they now accept explicit parameters
                tools.append(tool_metadata.tool_function)
                logger.info(f"[AGENT FACTORY] ✅ Found tool '{tool_name}' for role")
            else:
    ...
    def _enhance_task_with_context(self, task: str, context: dict[str, Any]) -> str:
        ...
        try:
            if isinstance(context, StandardizedContext):
                ...
                # Validate that critical fields are present before conversion
                if not context:
                    return task
                ...
                if missing_fields:
                    logger.warning(f"⚠️ Missing required context fields: {missing_fields}. Using fallback context creation.")
                    # Use fallback method for incomplete context
...
"""
                ...
                # Convert dict to standardized context, filtering out unwanted keys
                filtered_context = {k: v for k, v in context.items() 
                                  if k not in ['is_leadership_chat', 'is_main_chat', 'memory_context']}
                
                try:
                    standardized_context = StandardizedContext.from_dict(filtered_context)
                except ValueError as e:
                    logger.warning(f"⚠️ Failed to create standardized context: {e}, using fallback")
                    # Use fallback method if validation fails
                    user_id = context.get('user_id', 'unknown')
                    team_id = context.get('team_id', 'unknown')
                    chat_type = context.get('chat_type', 'unknown')
                    username = context.get('username', '')
...
EXAMPLES:
- get_team_overview(team_id="{standardized_context.team_id}", user_id="{standardized_context.user_id}")
- get_my_status(team_id="{standardized_context.team_id}", user_id="{standardized_context.user_id}")
- get_all_players(team_id="{standardized_context.team_id}")
...
ALWAYS:
- Pass team_id="{standardized_context.team_id}" explicitly to every tool that needs it
- Pass user_id="{standardized_context.user_id}" explicitly to every tool that needs it
- Use the exact values from the execution context above
"""
