        ...
        try:
            ...
            if not telegram_group_invite_base:
                try:
                    ...
                    team_config = config.get_team_config(self.team_id)
                    if team_config and team_config.main_chat_id:
                        invite_link = await self._create_telegram_invite_link(team_config.main_chat_id)
                    else:
                        error_msg = f"CRITICAL ERROR: Main chat ID not available for team {self.team_id}. Cannot generate invitation link."
                        logging.error(error_msg)
                        raise ValueError(error_msg)
                except Exception as e:
                    error_msg = f"CRITICAL ERROR: Failed to get team configuration for team {self.team_id}: {e}"
                    logging.error(error_msg)
                    raise ValueError(error_msg)
            else:
