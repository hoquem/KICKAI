...
class ToolRegistrationCheck(BaseCheck):
    ...
    async def execute(self, context: Dict[str, Any]) -> CheckResult:
        ...
        try:
            ...
            from core.registry_manager import get_registry_manager, RegistryManager
            ...
            return CheckResult(
                ...
                category=self.category,
                status=CheckStatus.FAILED,
                message=f"Import error during tool registration check: {str(e)}"
            )
            ...
            return CheckResult(
                ...
                category=self.category,
                status=CheckStatus.FAILED,
                message=f"Tool registration check failed: {str(e)}"
            ) 