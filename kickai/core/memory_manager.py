...
class KICKAIMemoryManager:
    ...
    def _initialize_crewai_memory(self):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"⚠️ CrewAI ShortTermMemory failed (ChromaDB issue): {short_term_error}"
                )
    ...
    def get_memory_for_agent(self, agent_role: AgentRole) -> Any:
        ...
        if memory_system is not None:
            ...
            logger.warning(
                f"⚠️ No memory system available for {agent_role.value}, using local cache"
            )
    ...
    def get_memory_for_entity(self, entity_type: EntityType) -> Any:
        ...
        if memory_system is None:
            logger.warning(
                f"⚠️ No memory system available for {entity_type.value}, using local cache"
            )
            return self._get_local_memory_proxy()

        return memory_system

    def store_player_memory(
        self,
        player_id: str,
        key: str,
        value: Any,
    ...
    def store_session_memory(
        self, session_id: str, key: str, value: Any, ttl: timedelta | None = None
    ):
        """
