...
class IPlayerReadRepository(ABC):
    ...
    @abstractmethod
    async def get_by_phone(self, phone: PhoneNumber, team_id: TeamId) -> dict[str, Any] | None:
        """Get player by phone number and team."""
        pass

    @abstractmethod
    async def get_by_user_id(self, user_id: UserId, team_id: TeamId) -> dict[str, Any] | None:
        """Get player by user ID and team."""
        pass

    @abstractmethod
    async def get_active_players(self, team_id: TeamId) -> list[dict[str, Any]]:
...
class IPlayerWriteRepository(ABC):
    ...
    @abstractmethod
    async def create_player(self, player_data: dict[str, Any], team_id: TeamId) -> dict[str, Any]:
        """Create new player."""
        pass

    @abstractmethod
    async def update_player(
        self, player_id: PlayerId, team_id: TeamId, updates: dict[str, Any]
    ) -> dict[str, Any] | None:
        """Update player information."""
        pass

    @abstractmethod
    async def set_player_status(self, player_id: PlayerId, team_id: TeamId, status: str) -> bool:
        """Set player status."""
        pass
...
class IPlayerApprovalRepository(ABC):
    ...
    @abstractmethod
    async def get_pending_approvals(self, team_id: TeamId) -> list[dict[str, Any]]:
...
class IPlayerRepository(
    IPlayerReadRepository, IPlayerWriteRepository, IPlayerApprovalRepository, IRepository
):
    """
