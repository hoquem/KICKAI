...
class AgentValidator:
    ...
    def validate_tool_output_capture_methods(self) -> ValidationResult:
        ...
        try:
            ...
            required_methods = {
                'execute', 'add_tool', 'remove_tool', 'get_tools',
                'is_enabled', 'get_config_summary'
            }
    ...
    def validate_method_calls(self) -> ValidationResult:
        ...
        try:
            ...
            if hasattr(ToolOutputCapture, 'clear_captured_outputs'):
                warnings.append("ToolOutputCapture has clear_captured_outputs method (should be on Mixin)")
            else:
                # This is actually correct - the method should be on the Mixin
                details['tool_capture_methods'] = [m for m in dir(ToolOutputCapture) if not m.startswith('_')]
            ...
            if hasattr(ConfigurableAgent, '__mro__'):
                mixin_in_chain = any('ToolOutputCaptureMixin' in str(base) for base in ConfigurableAgent.__mro__)
            ...
            try:
                ...
                if 'execute' not in crewai_agent_methods:
                    warnings.append("CrewAI Agent class does not have 'execute' method - should use Crew.kickoff() instead")
                
                # Check if 'kickoff' method exists on CrewAI Agent
                if 'kickoff' not in crewai_agent_methods:
                    warnings.append("CrewAI Agent class does not have 'kickoff' method - this is expected")
                ...
                if hasattr(ConfigurableAgent, 'execute'):
                    ...
                    import inspect
                    execute_method = getattr(ConfigurableAgent, 'execute')
                    if inspect.isfunction(execute_method):
                        try:
                            source = inspect.getsource(execute_method)
                            if 'self._crew_agent.execute(' in source:
                                errors.append("ConfigurableAgent.execute() calls self._crew_agent.execute() - this will fail")
    ...
    def validate_agent_factory(self) -> ValidationResult:
        ...
        try:
            ...
            required_methods = {
                'create_agent', 'create_all_agents', 'get_agent'
            }
