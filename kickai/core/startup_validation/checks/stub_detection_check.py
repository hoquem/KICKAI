...
class StubDetectionCheck(BaseCheck):
    ...
    description = "Validates that no stub classes are being used and all real implementations are loaded."
    ...
    async def execute(self, context=None) -> CheckResult:
        try:
            ...
            try:
                ...
                if not hasattr(router, 'route_command'):
                    stub_detections.append("AgenticMessageRouter missing route_command method")
                if not hasattr(router, 'convert_telegram_update_to_message'):
                    stub_detections.append("AgenticMessageRouter missing convert_telegram_update_to_message method")
                else:
                    implementation_validations.append("AgenticMessageRouter has all required methods")
            ...
            try:
                ...
                if 'SimpleMessageRouter' in tbs.__dict__:
                    stub_detections.append("TelegramBotService still contains SimpleMessageRouter stub")
                else:
                    implementation_validations.append("TelegramBotService uses real AgenticMessageRouter")
            ...
            try:
                ...
                if hasattr(manager, '_create_crew'):
                    implementation_validations.append("CrewLifecycleManager has real TeamManagementSystem creation")
                else:
            ...
            try:
                ...
                if hasattr(kickai.agents, 'TeamManagementSystem'):
                    ...
                    if 'minimal_stubs' in source:
                        stub_detections.append("TeamManagementSystem still imported from minimal_stubs")
                    else:
                        implementation_validations.append("TeamManagementSystem imported from real source")
