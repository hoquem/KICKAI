...
class ToolRegistrationCheck(BaseCheck):
    ...
    async def execute(self, context: dict[str, Any]) -> CheckResult:
        ...
        try:
            ...
            from kickai.agents.tool_registry import (
                ToolRegistry,
                get_tool_registry,
                initialize_tool_registry,
            )
            from kickai.core.registry_manager import RegistryManager, get_registry_manager
            ...
            # Test 2: Tool discovery using singleton
            ...
            logger.info(
                f"🔧 Tool discovery results: {total_tools} total tools, {enabled_tools} enabled"
            )
            ...
            logger.info(
                f"✅ Tool registration check passed: {total_tools} tools discovered across {len(features_with_tools)} features"
            )
