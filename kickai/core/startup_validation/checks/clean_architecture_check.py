...
class CleanArchitectureCheck(BaseCheck):
    ...
    async def execute(self, context=None) -> CheckResult:
        ...
        try:
            ...
            return CheckResult(
                ...
                name=self.name,
                category=self.category,
                message=f"Clean Architecture check failed: {e!s}",
                error=e,
            )
    ...
    async def _check_tool_registry_imports(self) -> list[str]:
        ...
        try:
            ...
            for i, line in enumerate(lines, 1):
                ...
                if (
                    ".domain.tools" in line_stripped
                    and "from kickai.features" in line_stripped
    ...
    async def _check_application_layer_structure(self) -> list[str]:
        ...
        if tools_found == 0:
            ...
            violations.append(
                f"Tool migration in progress: {tools_found}/{total_tools} tools in application layer ({migration_percentage:.1f}%)"
            )
        elif migration_percentage < 80:
            violations.append(
                f"Tool migration mostly complete: {tools_found}/{total_tools} tools in application layer ({migration_percentage:.1f}%) - continue migration"
            )
        else:
            # Migration is mostly complete, just warn about remaining domain tools
            if domain_tools_found > 0:
                violations.append(
                    f"Tool migration nearly complete: {tools_found}/{total_tools} tools in application layer ({migration_percentage:.1f}%) - {domain_tools_found} tools remaining in domain layer"
                )
