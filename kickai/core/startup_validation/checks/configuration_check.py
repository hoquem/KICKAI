...
class ConfigurationCheck(BaseCheck):
    ...
    async def execute(self, context: dict[str, Any] = None) -> CheckResult:
        ...
        try:
            ...
            has_pair = bool(getattr(config, "ai_model_simple", None)) and bool(
                getattr(config, "ai_model_advanced", None)
            )
            has_legacy = bool(getattr(config, "ai_model_name", None))
            if not has_pair and not has_legacy:
                missing_fields.append(
                    "AI model configuration (AI_MODEL_SIMPLE & AI_MODEL_ADVANCED required)"
                )
            elif has_legacy and not has_pair:
                logger.warning(
                    "AI_MODEL_NAME is deprecated. Please use AI_MODEL_SIMPLE and AI_MODEL_ADVANCED instead."
                )
