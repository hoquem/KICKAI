...
class Settings(BaseSettings):
    ...
    ai_provider: AIProvider = Field(
        default=AIProvider.GROQ, alias="AI_PROVIDER", description="AI provider to use"
    )

    # API Keys
    groq_api_key: str = Field(
        default="", alias="GROQ_API_KEY", description="Groq API key (REQUIRED for Groq)"
    )
    gemini_api_key: str = Field(
        default="", alias="GOOGLE_API_KEY", description="Google API key (REQUIRED for Gemini)"
    )
    openai_api_key: str = Field(
        default="", alias="OPENAI_API_KEY", description="OpenAI API key (REQUIRED for OpenAI)"
    )
    ...
    ai_temperature_creative: float = Field(
        default=0.7, description="AI temperature for creative tasks"
    )
    ...
    ai_max_tokens_creative: int = Field(
        default=1000, description="AI max tokens for creative tasks"
    )
    ...
    telegram_webhook_url: str | None = Field(
        default=None, alias="TELEGRAM_WEBHOOK_URL", description="Telegram webhook URL"
    )
    ...
    log_file_path: str | None = Field(
        default="logs/kickai.log", alias="LOG_FILE_PATH", description="Log file path"
    )
    ...
    def validate_ai_provider(cls, v):
        ...
        if isinstance(v, str):
            try:
                ...
                raise ValueError(
                    f"Invalid AI_PROVIDER: {v}. Supported: {[p.value for p in AIProvider]}"
                )
        return v
    ...
    def validate_firebase_credentials(cls, v, info):
        ...
        other_field = (
            ...
            if info.field_name == "firebase_credentials_file"
            else "firebase_credentials_file"
        )
