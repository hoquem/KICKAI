...
class DatabaseServiceHealthChecker(IServiceHealthChecker):
    ...
    async def check_health(self, service_name: str, service_instance: Any) -> ServiceHealth:
        ...
        try:
            ...
            has_async_test_conn = test_conn_attr is not None and inspect.iscoroutinefunction(
                test_conn_attr
            )
            if has_async_ping or has_async_test_conn:
                try:
                    ...
                    return ServiceHealth(
                        ...
                        response_time=response_time,
                        error_message=str(e),
                        metadata={"checker_type": "database", "connection_test_failed": True},
                    )
            ...
            return ServiceHealth(
                ...
                last_check=time.time(),
                response_time=response_time,
                metadata={"checker_type": "database", "interface_validation": True},
            )
    ...
    def supports_service(self, service_name: str) -> bool:
        ...
        return service_name in self.supported_services or any(
            keyword in service_name.lower()
            for keyword in ["database", "db", "store", "repository", "firestore"]
        )
...
class PlayerServiceHealthChecker(IServiceHealthChecker):
    ...
    def __init__(self):
        self.supported_services = {"PlayerService", "IPlayerService", "player_service"}
    ...
    async def check_health(self, service_name: str, service_instance: Any) -> ServiceHealth:
        ...
        try:
            ...
            missing_methods = [
                method for method in required_methods if not hasattr(service_instance, method)
            ]
            ...
            if hasattr(service_instance, "health_check"):
                try:
                    ...
                    return ServiceHealth(
                        ...
                        response_time=response_time,
                        error_message=str(e),
                        metadata={"checker_type": "player_service", "health_check_failed": True},
                    )
            ...
            return ServiceHealth(
                ...
                last_check=time.time(),
                response_time=response_time,
                metadata={"checker_type": "player_service", "interface_validation": True},
            )
    ...
    def supports_service(self, service_name: str) -> bool:
        """Check if this checker supports the service."""
        return service_name in self.supported_services or "player" in service_name.lower()
...
class TeamServiceHealthChecker(IServiceHealthChecker):
    ...
    def __init__(self):
        self.supported_services = {"TeamService", "ITeamService", "team_service"}
    ...
    async def check_health(self, service_name: str, service_instance: Any) -> ServiceHealth:
        ...
        try:
            ...
            if hasattr(service_instance, "health_check"):
                try:
                    ...
                    return ServiceHealth(
                        ...
                        response_time=response_time,
                        error_message=str(e),
                        metadata={"checker_type": "team_service", "health_check_failed": True},
                    )
            ...
            return ServiceHealth(
                ...
                last_check=time.time(),
                response_time=response_time,
                metadata={"checker_type": "team_service", "interface_validation": True},
            )
    ...
    def supports_service(self, service_name: str) -> bool:
        """Check if this checker supports the service."""
        return service_name in self.supported_services or "team" in service_name.lower()
...
class AgentServiceHealthChecker(IServiceHealthChecker):
    ...
    async def check_health(self, service_name: str, service_instance: Any) -> ServiceHealth:
        ...
        try:
            ...
            if "agent" in service_name.lower() or "crew" in service_name.lower():
                ...
                if hasattr(service_instance, "route_message") or hasattr(
                    service_instance, "process_message"
                ):
                    # If router methods exist, check that they are callable and configured
                    has_capability = callable(
                        getattr(service_instance, "route_message", None)
                    ) or callable(getattr(service_instance, "process_message", None))
                    response_time = time.time() - start_time
                    return ServiceHealth(
                        service_name=service_name,
            ...
            return ServiceHealth(
                ...
                metadata={
                    ...
                    "routing_capabilities": bool(
                        hasattr(service_instance, "route_message")
                        or hasattr(service_instance, "process_message")
                    ),
                },
            )
    ...
    def supports_service(self, service_name: str) -> bool:
        """Check if this checker supports the service."""
        return service_name in self.supported_services or any(
            keyword in service_name.lower() for keyword in ["agent", "crew", "router", "message"]
        )
...
class ExternalServiceHealthChecker(IServiceHealthChecker):
    ...
    def supports_service(self, service_name: str) -> bool:
        ...
        return service_name in self.supported_services or any(
            keyword in service_name.lower()
            for keyword in ["llm", "telegram", "firebase", "client", "provider", "bot"]
        )
