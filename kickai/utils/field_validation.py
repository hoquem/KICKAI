...
class FieldValidator:
    ...
    def validate_phone(phone: str) -> str:
        ...
        if not is_valid_phone(phone_clean):
            raise ValidationError(
                "Invalid phone number format. Please use UK format: " "+447XXXXXXXXX or 07XXXXXXXXX"
            )

        normalized = normalize_phone(phone_clean)
        if not normalized:
            raise ValidationError("Could not normalize phone number. Please check the format.")
    ...
    def validate_role(role: str, allow_admin: bool = True) -> str:
        ...
        if role_clean not in FieldValidator.VALID_ROLES:
            ...
            raise ValidationError(
                f"Invalid role '{role}'. Valid roles are: " f"{', '.join(sorted_roles)}"
            )
    ...
    def validate_emergency_contact_name(name: str) -> str:
        ...
        if len(name_clean) > 50:
            raise ValidationError("Emergency contact name cannot exceed 50 characters")
    ...
    def validate_medical_notes(notes: str) -> str:
        ...
        if len(notes_clean) > 500:
            raise ValidationError("Medical notes cannot exceed 500 characters")

        # Basic content validation - no HTML tags or suspicious content
        if re.search(r"<[^>]*>", notes_clean):
            raise ValidationError("Medical notes cannot contain HTML tags")
        ...
        for pattern in suspicious_patterns:
            if re.search(pattern, notes_clean, re.IGNORECASE):
                raise ValidationError(
                    "Medical notes contain invalid content. " "Please use plain text only."
                )
    ...
    def validate_field_for_entity(
        field: str, value: str, entity_type: str, current_user_is_admin: bool = False
    ) -> tuple[str, Any]:
        """
        ...
        # Team member-specific fields
            ...
            if field_clean == "role":
                return "role", FieldValidator.validate_role(
                    value, allow_admin=current_user_is_admin
                )
    ...
    def get_updatable_fields(entity_type: str) -> list[str]:
        ...
        """
        common_fields = ["phone", "email", "emergency_contact_name", "emergency_contact_phone"]
    ...
    def create_help_message(entity_type: str, current_user_is_admin: bool = False) -> str:
        ...
        help_text += "Example: /update email user@newdomain.com"
