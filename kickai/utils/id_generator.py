        ...
        with self._lock:
            ...
            while member_number in existing_numbers:
                ...
                if member_number > 99:  # Limit to 99 members
                    ...
                    hash_suffix = (
                        hashlib.md5(f"{name}{len(existing_numbers)}".encode())
                        .hexdigest()[:2]
                        .upper()
                    )
    ...
    def generate_match_id(
        ...
        with self._lock:
            ...
            try:
                ...
                logger.info(
                    f"Generated match ID '{final_id}' for {home_team} vs {away_team} on {match_date}"
                )
                return final_id
                ...
                self.used_match_ids.add(final_id)
                logger.info(
                    f"Generated fallback match ID '{final_id}' for {home_team} vs {away_team}"
                )
                return final_id
    ...
    def generate_training_id(self, team_id: str, session_type: str, date: str, time: str) -> str:
        ...
        with self._lock:
            try:
                ...
                logger.info(
                    f"Generated training ID '{training_id}' for {team_id} on {date} at {time}"
                )
                return training_id
...
def generate_player_id(
    name: str, team_id: str = "KTI", existing_ids: set[str] | None = None
) -> str:
...
def generate_football_team_id(team_name: str, league_info: str = "") -> str:
    ...
    logger.warning(
        "Using deprecated function generate_football_team_id, use generate_team_id instead"
    )
    return generate_team_id(team_name)


def generate_football_player_id(
    name: str, position: str, team_id: str, existing_ids: set[str] | None = None
) -> str:
    """Legacy function - use generate_member_id instead (thread-safe)."""
    logger.warning(
        "Using deprecated function generate_football_player_id, use generate_member_id instead"
    )
...
def generate_football_match_id(
    ...
    logger.warning(
        "Using deprecated function generate_football_match_id, use generate_match_id instead"
    )
...
def generate_football_training_id(team_id: str, session_type: str, date: str, time: str) -> str:
    ...
    logger.warning(
        "Using deprecated function generate_football_training_id, use generate_training_id instead"
    )
