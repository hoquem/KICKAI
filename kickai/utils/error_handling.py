...
def critical_system_error_handler(
    operation_name: str,
    re_raise_runtime: bool = True,
    re_raise_connection: bool = True
    ...
    def decorator(func: Callable[P, T]) -> Callable[P, T]:
        ...
        async def async_wrapper(*args: P.args, **kwargs: P.kwargs) -> T:
            try:
                ...
                logger.critical(f"💥 CRITICAL SYSTEM ERROR in {operation_name}: Database connection failed - {e}")
                logger.critical("🚨 This indicates a serious database connectivity failure")
                if re_raise_connection:
                    raise ConnectionError(f"CRITICAL SYSTEM ERROR in {operation_name}: Database connection failed - {e}")
                else:
                    raise
...
def command_registry_error_handler(func: Callable[P, T]) -> Callable[P, T]:
    ...
    async def async_wrapper(*args: P.args, **kwargs: P.kwargs) -> T:
        try:
            ...
            logger.critical(f"💥 CRITICAL SYSTEM ERROR: Unexpected error in command registry operation - {e}")
            logger.critical("🚨 This indicates an unexpected system failure")
            raise RuntimeError(
