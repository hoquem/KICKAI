    ...
    def _fallback_validation(self, phone: str) -> PhoneValidationResult:
        ...
        if len(digits_only) < LimitConstants.MIN_PHONE_DIGITS:
            return PhoneValidationResult(
                ...
                country_code="",
                national_number="",
                error_message=ValidationConstants.PHONE_TOO_SHORT_MSG.format(
                    min=LimitConstants.MIN_PHONE_DIGITS
                ),
            )
        ...
        if phone.startswith("+"):
            normalized = phone
            country_code = (
                phone[:3] if phone.startswith(ValidationConstants.UK_COUNTRY_CODE) else phone[:4]
            )
        else:
            # Assume UK number
