...
def validate_tool_context(required_keys: list[str]) -> dict[str, str]:
    ...
    raise ValueError(
        f"Context access is deprecated. Pass {required_keys} as direct parameters to tool."
    )
...
def get_context_for_tool(tool_name: str, required_keys: list[str] = None) -> dict[str, Any]:
    ...
    raise ValueError(
        f"Tool '{tool_name}' should receive parameters directly via function signature."
    )
