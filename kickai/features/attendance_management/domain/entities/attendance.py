...
class AttendanceSummary:
    ...
    def from_attendance_list(
        ...
        available = sum(
            1 for a in attendance_list if status_str(a) == AttendanceStatus.PRESENT.value
        )
        unavailable = sum(
            1 for a in attendance_list if status_str(a) == AttendanceStatus.ABSENT.value
        )
        maybe = sum(1 for a in attendance_list if status_str(a) == AttendanceStatus.MAYBE.value)
        no_response = sum(
            1 for a in attendance_list if status_str(a) == AttendanceStatus.NOT_RESPONDED.value
        )
