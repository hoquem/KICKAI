...
async def mark_availability_match(
    ...
    try:
        ...
        if not all([match_id, status]):
            return create_tool_response(False, "Match ID and status are required"
            )
        ...
        if not attendance_status:
            return create_tool_response(False, f"Invalid status. Use: {', '.join(status_mapping.keys())}"
            )
        ...
        if not attendance_service:
            return create_tool_response(False, "AttendanceService is not available"
            )
...
async def get_availability_all(telegram_id: int, team_id: str, username: str, chat_type: str, match_id: str) -> str:
    ...
    try:
        ...
        if not attendance_summary:
            return create_tool_response(True, "Operation completed successfully", data=f"📊 Match Availability: {match_id}\n\nNo availability responses recorded yet."
            )
...
async def record_attendance_match(
    ...
    try:
        ...
        if not all([match_id, player_id, status]):
            return create_tool_response(False, "Match ID, player ID, and status are required"
            )
        ...
        attendance_status = status_mapping.get(status.lower())
        if not attendance_status:
            return create_tool_response(False, f"Invalid attendance status. Use: {', '.join(status_mapping.keys())}"
            )
...
async def get_match_attendance(telegram_id: int, team_id: str, username: str, chat_type: str, match_id: str) -> str:
    ...
    try:
        ...
        if not attendance_records:
            return create_tool_response(True, "Operation completed successfully", data=f"📊 Match Attendance: {match_id}\n\nNo attendance records found."
            )
...
async def get_player_attendance_history(telegram_id: int, team_id: str, username: str, chat_type: str, player_id: Optional[str] = None) -> str:
    ...
    try:
        ...
        if not history:
            return create_tool_response(True, "Operation completed successfully", data=f"📊 Attendance History\n\nNo attendance history found for player {target_player_id}."
            )
