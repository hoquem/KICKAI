...
class FirestoreAttendanceRepository(AttendanceRepositoryInterface):
    ...
    async def get_by_player_and_match(
        ...
        try:
            ...
            logger.error(
                f"Failed to get attendance for player {player_id} and match {match_id}: {e}"
            )
            return None
    ...
    async def get_player_stats(self, player_id: str, team_id: str, year: int | None = None) -> dict:
        ...
        try:
            ...
            if year:
                year_str = str(year)
                attendance_list = [
                    a for a in attendance_list if a.match_date and a.match_date.startswith(year_str)
                ]
            ...
            response_rate = (
                ((total_matches - no_response) / total_matches * 100) if total_matches > 0 else 0.0
            )
