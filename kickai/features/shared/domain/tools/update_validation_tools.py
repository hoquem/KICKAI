...
class CommonUpdateValidator:
    ...
    def validate_phone_format(phone: str) -> str:
        """
        Validate and normalize UK phone number format using phonenumbers library.
        ...
        if not phone or not phone.strip():
            raise UpdateValidationError("Phone number cannot be empty")
    ...
    def validate_text_field(
        text: str, field_name: str, min_length: int = 1, max_length: int = 200
    ) -> str:
        """
        ...
        if len(text_stripped) > max_length:
            raise UpdateValidationError(f"{field_name} must be less than {max_length} characters")

        return text_stripped

    @staticmethod
    def validate_choice_field(
        value: str, field_name: str, valid_choices: list[str], case_sensitive: bool = False
    ) -> str:
        """
...
@tool
def validate_update_field_generic(
    field_type: str, field_value: str, validation_params: str = "{}"
) -> str:
    """
    ...
    try:
        ...
        return "❌ Validation Error: An unexpected error occurred during validation"


@tool
def check_field_uniqueness(
    collection_name: str, field_name: str, field_value: str, exclude_user_id: str = None
) -> str:
    """
    ...
    try:
        ...
        if exclude_user_id:
            existing_records = [
                record for record in existing_records if record.get("user_id") != exclude_user_id
            ]
        ...
        return "❌ Error checking uniqueness: Please try again"
...
@tool
def format_update_success_message(
    entity_type: str,
    entity_name: str,
    field_name: str,
    new_value: str,
