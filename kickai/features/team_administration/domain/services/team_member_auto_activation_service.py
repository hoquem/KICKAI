...
class TeamMemberAutoActivationService:
    ...
    async def process_new_leadership_chat_member(
        self, telegram_id: int, username: str, invite_context: dict[str, Any] | None = None
        ...
        try:
            ...
            invite_validation = await self._validate_invite_link(
                invite_context, telegram_id, username
            )

            if not invite_validation["valid"]:
                logger.warning(
                    f"❌ Invalid or missing invite for team member {username} (ID: {telegram_id})"
                )
            ...
            activation_result = await self._activate_team_member_from_invite(
                invite_validation["invite_data"], telegram_id, username
            )

            if activation_result.success:
                logger.info(
                    f"✅ Successfully auto-activated team member: {activation_result.member_name} (ID: {activation_result.member_id})"
                )

                # Step 3: Log successful activation for audit
                await self._log_activation_event(
                    activation_result, invite_validation["invite_data"]
                )

                return activation_result
            else:
                logger.error(
                    f"❌ Failed to activate team member for {username}: {activation_result.error}"
                )
                return activation_result
    ...
    async def _validate_invite_link(
        self, invite_context: dict[str, Any] | None, telegram_id: int, username: str
    ) -> dict[str, Any]:
        ...
        try:
            ...
            if invite_data:
                logger.info(
                    f"✅ Valid invite link found for team member {username}: {invite_data.get('invite_id')}"
                )
                return {
                    "valid": True,
    ...
    async def _activate_team_member_from_invite(
        self, invite_data: dict[str, Any], telegram_id: int, username: str
    ) -> TeamMemberActivationResult:
        ...
        try:
            ...
            team_member = await self._find_team_member_by_invite_data(
    ...
    async def _find_team_member_by_invite_data(
        self, member_id: str, member_phone: str | None, member_name: str
    ) -> TeamMember | None:
        """Find team member record using invite data."""
        try:
            # Try by member_id first (most reliable)
            try:
                team_member = await self.team_member_service.get_team_member_by_id(
                    member_id, self.team_id
                )
                if team_member:
            ...
            if member_phone:
                try:
                    team_member = await self.team_member_service.get_team_member_by_phone(
                        member_phone, self.team_id
                    )
            ...
            logger.warning(
                f"No team member found for ID: {member_id}, phone: {member_phone}, name: {member_name}"
            )
            return None
    ...
    async def _update_and_activate_team_member(
        self, team_member: TeamMember, telegram_id: int, username: str
    ) -> None:
        ...
        try:
            ...
            logger.info(
                f"✅ Team member {team_member.name} (ID: {team_member.member_id}) activated with Telegram ID: {telegram_id}"
            )

        except Exception as e:
            logger.error(
                f"❌ Error updating and activating team member {team_member.member_id}: {e}"
            )
            raise

    async def _log_activation_event(
        self, activation_result: TeamMemberActivationResult, invite_data: dict[str, Any]
    ) -> None:
        ...
        try:
            ...
            collection_name = get_team_specific_collection_name(
                self.team_id, "team_member_activation_logs"
            )
            ...
            logger.info(
                f"📝 Logged team member activation event for: {activation_result.member_name}"
            )
...
Thanks for understanding! ⚽
    ...
    def get_welcome_message_context(
        self, activation_result: TeamMemberActivationResult
    ) -> dict[str, Any]:
