...
async def add_team_member_simplified(
    ...
    try:
        ...
        # Get the consolidated team member management service from container
        management_service = container.get_service(TeamMemberManagementService)
        if not management_service:
            raise ServiceNotAvailableError("TeamMemberManagementService")
        ...
        if result.success:
            # Get team repository for team information
            team_repository = container.get_service(TeamRepositoryInterface)
            team = await team_repository.get_team_by_id(team_id) if team_repository else None
...
3. Contact system administrator if invite link issues persist"""
            ...
            return f"❌ Failed to add team member: {result.error_message}"

    except (MissingRequiredFieldError, PermissionError) as e:
        logger.warning(f"⚠️ Validation error in add_team_member_simplified: {e}")
        return f"❌ {str(e)}"
    
    except (RepositoryUnavailableError, ServiceNotAvailableError) as e:
        logger.error(f"❌ Service error in add_team_member_simplified: {e}")
        ...
        return f"❌ Team configuration error: {str(e)}"
