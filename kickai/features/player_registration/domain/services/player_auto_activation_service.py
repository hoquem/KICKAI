...
from kickai.core.exceptions import PlayerNotFoundError
...
class PlayerAutoActivationService:
    ...
    async def process_new_chat_member(
        ...
        username: str,
        chat_type: str,
        invite_context: dict[str, Any] | None = None,
    ) -> ActivationResult:
        """
        ...
        try:
            logger.info(
                f"🔄 Processing new chat member: {username} (ID: {telegram_id}) in {chat_type} chat"
            )
            ...
            if not invite_validation["valid"]:
                logger.warning(
                    f"❌ Invalid or missing invite for user {username} (ID: {telegram_id})"
                )
            ...
            if activation_result.success:
                logger.info(
                    f"✅ Successfully auto-activated player: {activation_result.player_name} (ID: {activation_result.player_id})"
                )
                ...
                logger.error(
                    f"❌ Failed to activate player for {username}: {activation_result.error}"
                )
                return activation_result
    ...
    async def _validate_invite_link(
        ...
        try:
            ...
            if invite_data:
                logger.info(
                    f"✅ Valid invite link found for {username}: {invite_data.get('invite_id')}"
                )
                return {
                    "valid": True,
    ...
    async def _find_player_by_invite_data(
        self, player_id: str, player_phone: str | None, player_name: str
    ) -> Player | None:
        ...
        try:
            ...
            if player_phone:
                try:
                    players = await self.player_service.get_players_by_phone(
                        player_phone, self.team_id
                    )
            ...
            logger.warning(
                f"No player found for ID: {player_id}, phone: {player_phone}, name: {player_name}"
            )
            return None
    ...
    async def _update_and_activate_player(
        self, player: Player, telegram_id: int, username: str
    ) -> None:
        ...
        try:
            ...
            logger.info(
                f"✅ Player {player.name} (ID: {player.player_id}) activated with Telegram ID: {telegram_id}"
            )
    ...
    async def _log_activation_event(
        self, activation_result: ActivationResult, invite_data: dict[str, Any]
    ) -> None:
        ...
        try:
            ...
            collection_name = get_team_specific_collection_name(
                self.team_id, "player_activation_logs"
            )
