...
class PlayerUpdateValidator:
    ...
    def validate_phone(cls, phone: str) -> str:
        ...
        if not phone or not phone.strip():
            raise PlayerUpdateValidationError("Phone number cannot be empty")
    ...
    def validate_emergency_contact(cls, contact: str) -> str:
        ...
        if len(contact.strip()) > 200:
            raise PlayerUpdateValidationError("Emergency contact must be less than 200 characters")
    ...
    def validate_medical_notes(cls, notes: str) -> str:
        ...
        if len(notes.strip()) > 500:
            raise PlayerUpdateValidationError("Medical notes must be less than 500 characters")
...
def update_player_information(
    ...
    try:
        logger.info(
            f"🔄 Player update request: user_id={user_id}, team_id={team_id}, field={field}"
        )
        ...
        try:
            ...
            logger.info("✅ Audit log created for player update")
