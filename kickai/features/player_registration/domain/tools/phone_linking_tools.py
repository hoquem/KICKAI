...
from kickai.utils.phone_validation import validate_phone_number, get_phone_variants, is_mobile_number


@tool
async def link_telegram_user_by_phone(phone: str, telegram_id: str, team_id: str, username: str = None) -> str:
    """
    ...
    try:
        ...
        try:
            ...
            return f"❌ Phone validation error. Please try again or contact team leadership."
        ...
        if linked_player:
            ...
            return "❌ No player record found with that phone number. Please contact team leadership."
            
    except Exception as e:
        logger.error(f"❌ Error in link_telegram_user_by_phone tool: {e}")
        import traceback
        logger.debug(f"❌ Traceback: {traceback.format_exc()}")
        return f"❌ Error linking account: {str(e)}"
