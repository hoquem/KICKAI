...
class MockTelegramBotService(TelegramBotServiceInterface):
    ...
    def __init__(
        ...
        if self.use_mock_ui:
            ...
            logger.info(f"💬 Main chat: {self.main_chat_id}, Leadership chat: {self.leadership_chat_id}")
            logger.info(f"🔍 Auto-detection mode: {'enabled' if not os.getenv('USE_MOCK_UI') else 'disabled (override set)'}")
    ...
    async def send_contact_share_button(self, chat_id: Union[int, str], text: str):
        ...
        try:
            ...
            logger.info(f"📱 Mock send_contact_share_button to {chat_context} chat ({chat_id}): {text}")
            ...
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=5)) as session:
                async with session.get(test_url) as response:
                    if response.status == 200:
                        ...
                        logger.info(f"📊 Mock UI stats: {stats.get('total_users', 0)} users, "
                ...
                async with session.get(users_url) as response:
                    if response.status == 200:
                        ...
                        if bot_user:
                            ...
                            logger.info(f"✅ Bot user {self.bot_username} already exists with ID {self.bot_user_id}")
                        else:
                            # Create bot user
                ...
                async with session.post(create_user_url, json=bot_user_data) as response:
                    if response.status == 200:
                        ...
                        if actual_id:
                            ...
                            logger.info(f"✅ Bot user created successfully with ID {actual_id}: {result}")
                        else:
                            logger.info(f"✅ Bot user created successfully: {result}")
                    else:
                        error_text = await response.text()
                        logger.warning(f"⚠️ Could not create bot user: {response.status} - {error_text}")
                        
        except Exception as e:
            logger.warning(f"⚠️ Error creating bot user: {e}")
            # Don't fail - the bot might work without explicit user creation

    async def send_announcement(self, chat_id: Union[int, str], message: str, **kwargs) -> Any:
        ...
        try:
            ...
            logger.info(f"📢 Mock send_announcement to {chat_context} chat ({chat_id}): {message[:50]}...")
    ...
    async def send_poll(self, chat_id: Union[int, str], question: str, options: List[str], **kwargs) -> Any:
    ...
    async def send_team_announcement(self, message: str, target_chat: str = "main", **kwargs) -> Any:
    ...
    async def broadcast_message(self, message: str, include_leadership: bool = True, **kwargs) -> List[Any]:
        ...
        try:
            ...
            if include_leadership:
                leadership_result = await self.send_message(self.leadership_chat_id, message, **kwargs)
