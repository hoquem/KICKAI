...
class TelegramBotService(TelegramBotServiceInterface):
    ...
    def _setup_handlers(self) -> None:
        ...
        try:
            ...
            logger.debug(f"✅ Set up {len(command_handlers)} command handlers and message handlers")
    ...
    async def _handle_new_chat_members(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            for member in update.message.new_chat_members:
                ...
                logger.info(
                    f"👤 Processing new member: {member.first_name} (@{member.username}, ID: {member.id})"
                )
                ...
                if invite_link:
                    await self._process_invite_link_join(
                        member, invite_link, update.effective_chat.id
                    )
                else:
    ...
    async def _process_invite_link_join(self, member, invite_link: str, chat_id: str):
        ...
        try:
            ...
            invite_data = await invite_service.validate_and_use_invite_link(
                invite_link=invite_link, user_id=str(member.id), username=member.username
            )
    ...
    async def _update_player_telegram_id(
        self, player_id: str, telegram_id: int, username: str = None
    ):
        ...
        try:
            ...
            if success:
                logger.info(
                    f"✅ Successfully updated player {player_id} with telegram_id {telegram_id}"
                )
            else:
                logger.error(
                    f"❌ Failed to update player {player_id} with telegram_id {telegram_id}"
                )

        except Exception as e:
            logger.error(f"❌ Error updating player telegram_id: {e}")

    async def _update_team_member_telegram_id(
        self, member_id: str, telegram_id: int, username: str = None
    ):
        ...
        try:
            ...
            if success:
                logger.info(
                    f"✅ Successfully updated team member {member_id} with telegram_id {telegram_id}"
                )
            else:
                logger.error(
                    f"❌ Failed to update team member {member_id} with telegram_id {telegram_id}"
                )
    ...
    async def start_polling(self) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"✅ Bot connected successfully: @{bot_info.username} (ID: {bot_info.id})"
                )
            except Exception as e:
        ...
        try:
            ...
            if update.effective_message:
                ...
                if update.effective_message.text:
                    logger.info(
                        f"🔍 DEBUG: Text: {update.effective_message.text[:MESSAGE_PREVIEW_LENGTH]}..."
                    )
        except Exception as e:
