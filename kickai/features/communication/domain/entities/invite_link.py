...
class InviteLink:
    ...
    def _validate(self):
        ...
        if not isinstance(self.status, InviteLinkStatus):
            raise ValueError(
                f"Invalid status type: {type(self.status)}. Must be InviteLinkStatus enum"
            )
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            if isinstance(self.status, InviteLinkStatus)
            else self.status,
    ...
    def _repr_(self) -> str:
        ...
        return (
            f"InviteLink(invite_id='{self.invite_id}', team_id='{self.team_id}', "
