...
class UserFlowAgent:
    ...
    async def handle_registered_user_flow(self, message) -> AgentResponse:
        return await self._format_registered_user_message(message.user_id, message.team_id, message.username)
    ...
    async def determine_user_flow(self, user_id: str, chat_type: ChatType) -> UserFlowDecision:
        ...
        return UserFlowDecision.REGISTERED_USER if is_registered else UserFlowDecision.UNREGISTERED_USER
