...
class IntentClassificationStep(PipelineStep):
    ...
    async def execute(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        return {**context, "intent_result": {"intent": intent, "confidence": 0.9, "entities": {}}}
...
class AgentSelectionStep(PipelineStep):
    ...
    async def execute(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        return {**context, "selected_agent": selected_agent}

    def _select_agent(
        self, intent_result: dict, execution_context: dict, available_agents: dict
    ) -> Any:
        ...
        if intent.startswith("command_"):
            ...
            if command in ["myinfo", "status"]:
                if chat_type == "main_chat":
                    ...
                    return available_agents.get(
                        AgentRole.PLAYER_COORDINATOR
                    ) or available_agents.get(AgentRole.MESSAGE_PROCESSOR)
                else:
        ...
        if intent == "help_request":
            ...
            if chat_type == "main_chat":
                ...
                return available_agents.get(AgentRole.MESSAGE_PROCESSOR)
            return available_agents.get(AgentRole.ONBOARDING_AGENT) or available_agents.get(
                AgentRole.PLAYER_COORDINATOR
            )
...
class TaskExecutionStep(PipelineStep):
    ...
    async def execute(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if not validate_context_data(execution_context, "base"):
                logger.error("❌ [TASK EXECUTION] Context validation failed for execution context")
                return {
                    **context,
            ...
            validated_result = await self._validate_agent_output(
                agent_result, {"tool_outputs": tool_outputs}
            )

            logger.info("✅ [TASK EXECUTION] Task completed successfully")
            return {**context, "execution_result": validated_result, "tool_outputs": tool_outputs}

        except Exception as e:
            logger.error(f"❌ [TASK EXECUTION] Task execution failed: {e}")
            return {**context, "execution_result": f"Task execution failed: {e!s}"}
    ...
    def _extract_agent_context(self, execution_context: dict) -> dict:
        ...
        try:
            ...
            logger.info(
                f"🔍 [TASK EXECUTION] Extracted context: team_id={team_id}, user_id={user_id}, chat_type={chat_type}, telegram_username={telegram_username}"
            )
        ...
        if hasattr(agent_result, "tools_results") and agent_result.tools_results:
            for tool_result in agent_result.tools_results:
                if (
                    ...
                    logger.debug(
                        f"🔍 [TASK EXECUTION] Captured {tool_name} output from CrewAI tools_results"
                    )
    ...
    async def _validate_agent_output(self, agent_result: Any, context: dict) -> str:
        ...
        try:
            ...
            if tool_outputs:
                ...
                logger.debug("🔧 [VALIDATION] No tools used - agent provided direct response")
...
class SimplifiedOrchestrationPipeline:
    ...
    def __init__(self, llm=None):
        ...
        self.llm = llm
        self.steps = [IntentClassificationStep(), AgentSelectionStep(), TaskExecutionStep()]
    ...
    async def execute_task(
        ...
        try:
            ...
            logger.info(
                f"🚀 [SIMPLIFIED ORCHESTRATION] Starting pipeline execution for: {task_description[:50]}..."
            )
            ...
            for step in self.steps:
                try:
                    logger.info(
                        f"📋 [SIMPLIFIED ORCHESTRATION] Executing step: {step.get_step_name()}"
                    )
                    ...
                    logger.info(
                        f"✅ [SIMPLIFIED ORCHESTRATION] Step {step.get_step_name()} completed"
                    )

                except Exception as e:
                    logger.error(
                        f"❌ [SIMPLIFIED ORCHESTRATION] Step {step.get_step_name()} failed: {e}"
                    )
                    context["error"] = str(e)
                    break
    ...
    def _generate_final_response(self, context: dict) -> str:
        ...
        if context.get("error"):
            self.pipeline_analytics["failed_executions"] += 1
            logger.error(
                f"❌ [SIMPLIFIED ORCHESTRATION] Pipeline execution failed: {context['error']}"
            )
