...
class NLPProcessor(ConfigurableAgent):
    ...
    def __init__(self, team_id: str):
        ...
        try:
            ...
            super().__init__(agent_role=AgentRole.NLP_PROCESSOR, team_id=team_id)
            ...
            raise AgentInitializationError("NLP_PROCESSOR", f"Initialization failed: {e!s}")
...
def _handle_tool_error(tool_name: str, error: Exception, error_type: str = "unexpected") -> str:
    ...
    if isinstance(error, ToolValidationError):
        ...
        return create_json_response(
            ResponseStatus.ERROR, message=f"Invalid input for {tool_name}: {error}"
        )
    elif isinstance(error, AgentExecutionError):
        logger.error(f"❌ Agent execution error in {tool_name}: {error}")
        return create_json_response(
            ResponseStatus.ERROR, message=f"Execution failed during {tool_name}: {error}"
        )
    elif isinstance(error, InputValidationError):
        logger.warning(f"⚠️ Input validation error in {tool_name}: {error}")
        return create_json_response(
            ResponseStatus.ERROR, message=f"Input validation failed for {tool_name}: {error}"
        )
    else:
...
async def routing_recommendation_domain(
    telegram_id: int, team_id: str, username: str, chat_type: str, intent_data: str, **kwargs
) -> str:
    """
    ...
    try:
        ...
        request = (
            intent_data if intent_data and isinstance(intent_data, str) else "No request provided"
        )
        ...
        log_tool_execution(
            "routing_recommendation_domain", {"team_id": team, "request_length": len(request)}, True
        )
        ...
        return create_json_response(ResponseStatus.ERROR, message=f"Analysis failed: {e!s}")


async def analyze_update_context_domain(
    telegram_id: int, team_id: str, username: str, chat_type: str, message: str, **kwargs
) -> str:
    """
    ...
    try:
        ...
        if not validate_prompt_context("update_context", context):
            return create_json_response(
                ResponseStatus.ERROR, message="Invalid context for update analysis"
            )
    ...
    **kwargs,
    ...
    try:
        ...
        if not validate_prompt_context("permission_validation", context):
            return create_json_response(
                ResponseStatus.ERROR, message="Invalid context for permission validation"
            )

        permission_prompt = render_prompt("permission_validation", context)

        return create_json_response(
            ResponseStatus.SUCCESS, data={"permission_prompt": permission_prompt}
        )
