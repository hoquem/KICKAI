...
class AgenticMessageRouter:
    ...
    async def route_contact_share(self, message: TelegramMessage) -> AgentResponse:
        ...
        try:
            ...
            if not message.contact:
                return self._create_error_response(
                    ERROR_MESSAGES["NO_CONTACT_INFO"], "Missing contact data"
                )
            ...
            if not phone:
                return self._create_error_response(
                    "No phone number in contact", "Missing phone number"
                )
    ...
    def set_chat_ids(self, main_chat_id: str, leadership_chat_id: str) -> None:
        ...
        try:
            ...
            try:
                validated_main_chat_id = validate_string_input(
                    main_chat_id, "Main Chat ID", allow_empty=False
                )
                validated_leadership_chat_id = validate_string_input(
                    leadership_chat_id, "Leadership Chat ID", allow_empty=False
                )
                ...
                logger.info(
                    f"Chat IDs set - Main: {validated_main_chat_id}, Leadership: {validated_leadership_chat_id}"
                )
    ...
    def _create_error_response(self, message: str, error: str) -> AgentResponse:
        ...
        """
        try:
            return AgentResponse(success=False, message=message, error=error)
