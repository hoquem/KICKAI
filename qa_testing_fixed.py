#!/usr/bin/env python3
"""
Fixed QA Testing Script for KICKAI Bot
...
class KICKAIQATester:
    ...
    async def setup(self):
        ...
        await self.page.goto("http://localhost:8000/mock_tester.html")
    ...
    async def send_message(self, message: str) -> str:
        ...
        try:
            ...
            if not input_found:
                print(f"❌ Could not find input field for message: {message}")
                return ""
            ...
            for selector in message_selectors:
                try:
                    ...
                    if messages:
                        # Get the last few messages to find the bot response
                        for i in range(min(5, len(messages))):
                            message_element = messages[-(i+1)]
                            text = await message_element.text_content()
                            if text and text.strip() and not text.strip().startswith(message):
                                return text.strip()
                except:
                    continue
    ...
    async def test_help_commands(self):
        ...
        await self.end_test("/help", "Basic help command", 
                           TestStatus.PASSED if response and ("help" in response.lower() or "command" in response.lower()) else TestStatus.FAILED,
                           response_content=response,
                           expected_patterns=["help", "command"])
        ...
        await self.end_test("/version", "Version information",
                           TestStatus.PASSED if response and ("version" in response.lower() or "kickai" in response.lower()) else TestStatus.FAILED,
                           response_content=response,
        ...
        await self.end_test("/ping", "Bot status check",
                           TestStatus.PASSED if response and ("pong" in response.lower() or "online" in response.lower() or "ping" in response.lower()) else TestStatus.FAILED,
                           response_content=response)
    ...
    async def test_team_administration_commands(self):
        ...
        await self.end_test("/addplayer", "Add new player",
                           TestStatus.PASSED if response and ("player" in response.lower() or "added" in response.lower() or "created" in response.lower()) else TestStatus.FAILED,
                           response_content=response,
        ...
        await self.end_test("/addmember", "Add team member",
                           TestStatus.PASSED if response and ("member" in response.lower() or "added" in response.lower() or "created" in response.lower()) else TestStatus.FAILED,
                           response_content=response,
    ...
    async def test_player_update_commands(self):
        ...
        await self.end_test("/update", "Update phone number",
                           TestStatus.PASSED if response and ("updated" in response.lower() or "phone" in response.lower()) else TestStatus.FAILED,
                           response_content=response,
        ...
        await self.end_test("/update", "Update position",
                           TestStatus.PASSED if response and ("updated" in response.lower() or "position" in response.lower()) else TestStatus.FAILED,
                           response_content=response,
        ...
        await self.end_test("/update", "Invalid field update",
                           TestStatus.PASSED if response and ("error" in response.lower() or "invalid" in response.lower()) else TestStatus.FAILED,
                           response_content=response)
    ...
    async def test_permission_commands(self):
        ...
        await self.end_test("Permission", "Unauthorized command access",
                           TestStatus.PASSED if response and ("denied" in response.lower() or "unauthorized" in response.lower() or "not found" in response.lower()) else TestStatus.FAILED,
                           response_content=response)
        ...
        await self.end_test("Invalid Command", "Non-existent command",
                           TestStatus.PASSED if response and ("not found" in response.lower() or "unknown" in response.lower() or "error" in response.lower()) else TestStatus.FAILED,
                           response_content=response)
    ...
    async def test_error_handling(self):
        ...
        await self.end_test("Error Handling", "Malformed command",
                           TestStatus.PASSED if response and ("error" in response.lower() or "missing" in response.lower()) else TestStatus.FAILED,
                           response_content=response)
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        for command, results in command_results.items():
            ...
            skipped = len([r for r in results if r.status == TestStatus.SKIPPED.value])
        ...
        total_passed = len([r for r in self.test_results if r.status == TestStatus.PASSED.value])
        total_failed = len([r for r in self.test_results if r.status == TestStatus.FAILED.value])
